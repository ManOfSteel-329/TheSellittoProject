...
function App() {
  ...
  const [positionCount, setPositionCount] = useState(3); // 👈 New: position columns
  ...
  const positionFields = ['first', 'second', 'third', 'fourth', 'fifth', 'sixth'];
  const positionHeaders = ['1st', '2nd', '3rd', '4th', '5th', '6th'];

  const handleRaceNumberChange = (e) => { 
    const newTotal = parseInt(e.target.value);
    setTotalRaces(newTotal);
    setPredictions(Array(newTotal).fill({
      first: '', second: '', third: '', fourth: '', fifth: '', sixth: ''
    }));
  };

  const handlePositionCountChange = (e) => {
    setPositionCount(parseInt(e.target.value));
  };
  ...
  const getFileName = () => `${selectedTrack}_${selectedTalent}.json`;

  const handleSelectionChange = async () => {
    if (!selectedTrack || !selectedTalent) return;
  };
  ...
  const handleDisplay = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:3001/api/predictions/${getFileName()}`);
      if (response.ok) {
    ...
    >
      ...
      <div className="flex gap-8">
        <div className="w-1/2 space-y-4">
          ...
          <div className="flex items-center gap-4">
            <label className="w-32">Positions:</label>
            <select 
              className="w-full p-2 border rounded bg-gray-200 text-black"
              value={positionCount}
          ...
          <table className="w-full border-collapse border">
            <thead>
              <tr>
                ...
                {positionHeaders.slice(0, positionCount).map(header => (
            ...
            <tbody>
              {predictions.map((row, index) => (
                ...
                >
                  ...
                  {positionFields.slice(0, positionCount).map((field) => (
...
// function App() {
//   const [totalRaces, setTotalRaces] = useState(12); // 👈 New state for total races
//   const [predictions, setPredictions] = useState(
...
//     const newTotal = parseInt(e.target.value);
//     setTotalRaces(newTotal);
//     setPredictions(Array(newTotal).fill({ first: '', second: '', third: '', fourth: '' }));
//   };
